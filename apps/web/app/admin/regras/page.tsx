...
const tipoConfig: Record<string, { label: string; bg: string; text: string }> = {
  ROTEAMENTO: { label: 'Roteamento', bg: 'bg-brand-blue-soft', text: 'text-brand-blue' },
  VALIDACAO: { label: 'Validação', bg: 'bg-brand-purple-soft', text: 'text-brand-purple' },
  PRAZO: { label: 'Prazo', bg: 'bg-brand-amber-soft', text: 'text-brand-amber' },
  NOTIFICACAO: { label: 'Notificação', bg: 'bg-surface-1', text: 'text-fg-2' },
  BLOQUEIO: { label: 'Bloqueio', bg: 'bg-brand-red-soft', text: 'text-brand-red' },
  QUORUM: { label: 'Quórum', bg: 'bg-brand-green-soft', text: 'text-brand-green' },
}
  ...
  return (
    <div className="p-6 space-y-5 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-fg-1">Regras de Tramitação</h1>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { label: 'Ativas', value: regras.filter(r => r.ativo).length, color: 'text-brand-green' },
          { label: 'Inativas', value: regras.filter(r => !r.ativo).length, color: 'text-fg-3' },
          { label: 'Tipos distintos', value: new Set(regras.map(r => r.tipo)).size },
        ].map(stat => (
          <div key={stat.label} className="bg-surface-1 border border-line rounded-lg p-3 text-center">
            <div className={`text-[22px] font-bold font-mono ${stat.color || 'text-fg-1'}`}>{stat.value}</div>
            <div className="text-[11px] text-fg-3 mt-0.5">{stat.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-2">
        {regras.sort((a, b) => b.prioridade - a.prioridade).map(regra => {
          const tc = tipoConfig[regra.tipo] ?? { label: regra.tipo, bg: 'bg-surface-2', text: 'text-fg-2' }
          ...
          return (
            <div
              ...
              className={`border rounded-lg transition-colors ${
                regra.ativo ? 'border-line bg-surface-1' : 'border-line bg-surface-0 opacity-60'
              }`}
            >
              {/* Header da regra */}
              <div
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[13px] font-medium text-fg-1">{regra.nome}</div>
                  <div className="text-[11px] text-fg-3 mt-0.5 truncate">{regra.descricao}</div>
                </div>
                <div className="flex items-center gap-3 flex-shrink-0">
                  {regra.tipoMateria && (
                    <span className="font-mono text-[10px] bg-surface-0 border border-line px-2 py-0.5 rounded text-fg-3">
                      {regra.tipoMateria}
                    </span>
                  )}
                  <span className="text-[10px] text-fg-3">p={regra.prioridade}</span>
                  <span className="text-[10px] text-fg-3 font-mono">v{regra.versao}</span>
                  <button
                    onClick={(e) => { e.stopPropagation(); toggleAtivo(regra.id) }}
                    className={`transition-colors ${regra.ativo ? 'text-brand-green' : 'text-fg-3'}`}
                  >
                    {regra.ativo ? <ToggleRight size={18} /> : <ToggleLeft size={18} />}
                  </button>
                  {isExpanded ? <ChevronUp size={14} className="text-fg-3" /> : <ChevronDown size={14} className="text-fg-3" />}
                </div>
              </div>
                ...
                <div className="px-5 pb-4 pt-0 border-t border-line">
                  <div className="grid grid-cols-2 gap-4 mt-4">
                    <div>
                      ...
                      <pre className="bg-surface-0 border border-line rounded-md p-3 text-[11px] font-mono text-fg-2 overflow-x-auto">
                  ...
                  <div className="flex items-center gap-3 mt-4">
                    <button className="text-[12px] border border-line text-fg-2 hover:text-fg-1 px-3 py-1.5 rounded-md transition-colors">
                      Editar
                    </button>
                    <button className="text-[12px] border border-line text-fg-2 hover:text-fg-1 px-3 py-1.5 rounded-md transition-colors">
                      Duplicar
                    ...
                    <span className="text-[11px] text-fg-3 ml-auto">Atualizado em {regra.atualizadoEm}</span>
                  </div>
                </div>
              )}
            </div>
